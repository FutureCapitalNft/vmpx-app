...
const Footer = ({ tokenAddress, contractAddress: mainAddress, projectId = 'xen' }) => {
  ...
  const maybeTokenAddress = tokenAddress && tokenAddress.toString().startsWith('0x') ? tokenAddress: '';
  ...
  const tokenBlockExplorerUrl = typeof maybeTokenAddress === 'string'
    && currentNetwork?.explorerUrl
    && `${currentNetwork.explorerUrl}address/${maybeTokenAddress}`;


  const contractAddressString = projectId === 'xen' || projectId === 'xenft'
    ? `XEN: ${((maybeContractAddress || '').toString() || '-').slice(0, 12)}`
    : `${project.name}: ${((maybeContractAddress || '').toString() || '-').slice(0, 12)}`;

  const tokenAddressString = `${project.tokenSymbol || ''} token: ${((maybeTokenAddress || '').toString() || '-').slice(0, 12)}`;
      ...
      <Grid container sx={{ alignItems: 'center', pt: 2 }} >
        ...
        <Grid item xs={12} md={6} >
          {!isHomePage && <Typography variant="body2"
            ...
            {maybeTokenAddress && <Tooltip title={`Explore ${project.tokenSymbol} Token and Transactions`}>
                  <StyledAnchor href={tokenBlockExplorerUrl}
                                target="_blank"
                                rel="noreferrer"
                                style={{
                                  display: 'unset',
                                }}>
                      &nbsp;&bull;&nbsp;{tokenAddressString}
                  </StyledAnchor>
              </Tooltip>}
          </Typography>}
